...
interface InviteUsersFormProps {
    onCreated: (invites: FreeCreditInvite[]) => void
...
export function InviteUsersForm({ onCreated, notify, setError }: InviteUsersFormProps) {
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        setError(null)
        try {
            const created = await apiService.createFreeCreditInvites({
                emails,
            ...
            notify({
                tone: 'success',
                title: t('admin.creditCodes.invites.toastCreated'),
                message: t('admin.creditCodes.invites.toastCreatedCount', { count: created.length }),
            })
            onCreated(created)
        } catch (err) {
            const message = err instanceof Error ? err.message : t('admin.creditCodes.errors.createInvites')
            setError(message)
            notify({ tone: 'error', title: t('admin.creditCodes.invites.toastCreateFailed'), message })
        } finally {
            setSending(false)
        }
    }
    ...
    return (
        <form className="flex flex-col gap-4" onSubmit={(event) => void handleSubmit(event)}>
            <Field
                label={t('admin.creditCodes.invites.emailsLabel')}
                error={tooMany ? t('admin.creditCodes.invites.tooMany', { max: MAX_EMAILS }) : undefined}
                helper={t('admin.creditCodes.invites.emailsHelper', { count: emails.length })}
            >
                <Textarea
                    value={emailsRaw}
                    onChange={(event) => setEmailsRaw(event.target.value)}
                    placeholder={t('admin.creditCodes.invites.emailsPlaceholder')}
                />
            </Field>
            ...
            <div className="flex justify-end">
                ...
                >
                    {t('admin.creditCodes.invites.create')}
                </Button>
            </div>
        </form>
    )
