...
import { Ban, Coins, Loader2, Mail, Pencil } from 'lucide-react'
...
interface EmailCreditGrantsListProps {
    grants: EmailCreditGrant[]
    loading: boolean
    mutatingId: number | null
    onDisable: (grant: EmailCreditGrant) => void
    onEdit: (grant: EmailCreditGrant) => void
    hasMore: boolean
    loadingMore: boolean
    onLoadMore: () => void
}

export function EmailCreditGrantsList({
    grants,
    loading,
    mutatingId,
    onDisable,
    ...
    if (grants.length === 0) {
        return (
            <EmptyState
                ...
                message={loading ? t('admin.creditCodes.emailGrants.loading') : t('admin.creditCodes.emailGrants.empty')}
                secondaryText={loading ? undefined : t('admin.creditCodes.emailGrants.emptyHint')}
            />
        )
    }
        ...
        <div className="flex flex-col gap-3">
            <ul className="flex flex-col gap-2">
                {grants.map((grant) => {
                    ...
                    const busy = mutatingId === grant.grant_id
                    return (
                        <li
                            key={grant.grant_id}
                        ...
                        >
                            <div className="flex min-w-0 flex-col gap-1">
                                ...
                                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 font-ui text-[12px] text-parchment-400">
                                    ...
                                    {grant.label && <span className="truncate">{grant.label}</span>}
                                    ...
                                    {grant.email_delivery_status && (
                                        <span>
                                            {t('admin.creditCodes.emailGrants.deliveryStatus', {
                                                status: t(`admin.creditCodes.emailGrants.delivery.${grant.email_delivery_status}`, {
                                                    defaultValue: grant.email_delivery_status,
                                                }),
                                            })}
                                        </span>
                                    )}
