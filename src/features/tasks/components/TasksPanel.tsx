/**
 * Shared body for the background-tasks UI. Rendered in two hosts: the desktop
 * `SidebarTasksMenu` popover (rises from the rail) and the mobile `TasksDrawer`
 * (full-height slide-in). Owns the tab control, the manual-refresh action, the
 * inline-expandable task rows and the attached-card modal, so both hosts stay
...
function statusTone(status: BackgroundTaskPublic['status']): StatusTone {
    return status === 'completed' ? 'live' : status === 'failed' || status === 'canceled' ? 'nsfw' : 'arcane'
}
        ...
        <>
            <div className={cx('flex flex-col', dense ? 'gap-2.5' : 'gap-4')}>
                <div className="flex items-center gap-2">
                    <div
                        className="grid flex-1 grid-cols-3 gap-1 rounded-lg border border-parchment-50/10 bg-ink-900/45 p-1"
                        role="group"
                        aria-label={t('tasksDrawer.filterAria')}
                    >
                        {TAB_ORDER.map((tab) => (
                            <button
                                key={tab}
                    ...
                    <IconButton
                        label={t('tasksDrawer.actions.refresh')}
                        disabled={isRefreshing}
                        onClick={() => void handleRefresh()}
                    >
                        <Icon icon={RefreshCw} size={16} className={isRefreshing ? 'animate-spin' : undefined} />
                    </IconButton>
                </div>
                ...
                {visibleTasks.length === 0 ? (
                    ...
                    <div className={cx('flex flex-col', dense ? 'gap-2' : 'gap-2.5')}>
    ...
    return (
        <div className="overflow-hidden rounded-lg border border-parchment-50/10 bg-ink-800/60">
            <button
                ...
                aria-label={t(expanded ? 'tasksDrawer.collapse' : 'tasksDrawer.expand', { title })}
                onClick={onToggle}
                className="flex w-full items-center gap-3 px-3 py-2.5 text-left transition-colors hover:bg-parchment-50/[.04]"
            >
                <span className={cx('inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md', TILE_TONE[tone])}>
                    <Icon icon={IconCmp} size={17} className={isWorking ? 'animate-spin' : undefined} />
                </span>
                <span className="min-w-0 flex-1 truncate font-ui text-sm font-semibold text-parchment-50">{title}</span>
                <span className="flex shrink-0 items-center gap-1.5">
                    {task.status === 'completed' && duration && (
                        <span className="font-mono text-[10px] text-parchment-400">{duration}</span>
                    )}
                    <Badge tone={tone}>{statusCopy(task.status, t)}</Badge>
                    <Icon
                        icon={ChevronDown}
                        size={15}
                        className={cx('text-parchment-400 transition-transform duration-200', expanded && 'rotate-180')}
                    />
                </span>
            </button>
                ...
                <div className="min-h-0 overflow-hidden">
                    <div className="flex flex-col gap-3 px-3 pb-3 pt-2">
                        ...
                        <button
                            ...
                            className="inline-flex max-w-full items-center gap-1 self-start rounded-sm font-ui text-[11px] text-parchment-400 underline-offset-2 hover:text-parchment-50 hover:underline"
                            onClick={onOpenCard}
                        >
