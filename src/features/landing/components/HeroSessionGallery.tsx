...
import { Button, Eyebrow, Icon, IconButton, cx, gradientFor } from '@/ui/primitives'
...
export function HeroSessionGallery({ sessions, onOpen, onBeginNew }: HeroSessionGalleryProps) {
    ...
    return (
        <section className="relative" aria-label="Recent chats and adventures" data-testid="hero-session-gallery">
            {showControls && (
            ...
            >
                {visibleSessions.map((session) => (
                    <div
                        ...
                        role="listitem"
                    >
                        <HeroSessionSlide session={session} onOpen={() => onOpen(session)} onBeginNew={onBeginNew} />
                    </div>
                ))}
            </div>
...
function HeroSessionSlide({ session, onOpen, onBeginNew }: HeroSessionSlideProps) {
    const isChat = session.kind === 'chat'
    const modeMeta = MODE_META[session.kind]
    const media = useAuthenticatedMediaUrl(session.imageUrl ?? undefined, 'image/*')
    const imageSrc = media.error ? undefined : media.src
    const hasImage = Boolean(imageSrc)
    const monogram = session.title.charAt(0).toUpperCase() || '?'
    const eyebrow = ['Tonight', modeMeta.label, session.context].filter(Boolean).join(' · ')
    const actionLabel = isChat ? modeMeta.resumeLabel : 'Continue the tale'
    const actionIcon = isChat ? MessageCircle : Play
    const fallback =
        isChat
            ? 'The conversation is waiting where you left it.'
            : 'The next turn is waiting at the edge of the scene.'
        ...
        >
            <div className="pointer-events-none absolute inset-0" aria-hidden="true">
                ...
                <div
                    className={cx(
                        ...
                        isChat ? 'ring-arcane-300/15' : 'ring-ember-300/15',
                    )}
                />
            </div>
            ...
            <div className="relative z-[2] flex max-w-full flex-col justify-center gap-4 p-7 sm:p-12 md:max-w-[64%]">
                <div className="flex flex-wrap items-center gap-2.5">
                    <Eyebrow tone={modeMeta.tone}>{eyebrow}</Eyebrow>
                    <ModeBadge mode={session.kind} />
                </div>
                ...
                <p className="m-0 font-mono text-[12px] tracking-wide text-parchment-300">
                    {session.playingAs ? `${session.meta} · playing as ${session.playingAs}` : session.meta}
                </p>
                <div className="flex flex-wrap items-center gap-3 pt-1">
                    <Button
                        kind={isChat ? 'arcane' : 'primary'}
                        size="lg"
